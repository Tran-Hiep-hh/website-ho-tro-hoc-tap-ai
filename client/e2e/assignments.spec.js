...
test("teacher assigns a frozen Quiz; student resumes, submits and teacher reviews real results", async ({ page, browser, request }) => {
  ...
  try {
    ...
    await expect(page.getByRole("button", { name: "Xem kết quả", exact: true })).toHaveCount(0);
    await page.getByRole("button", { name: "Danh sách bài Quiz", exact: true }).click();
    await expect(page.getByText("Đã hủy", { exact: true })).toHaveCount(0);
    await page.getByRole("row", { name: "Mở bài Quiz Kiểm tra chương 1", exact: true }).click();
    await expect(page).toHaveURL(/\/details$/);
    await expect(page.getByRole("heading", { name: "Thông tin bài Quiz", exact: true })).toHaveCount(0);
    await page.getByRole("button", { name: "Chỉnh sửa", exact: true }).click();
    await page.getByRole("textbox", { name: "Giải thích câu 1", exact: true }).fill("Giải thích được giáo viên sửa trong bản nháp.");
    await page.getByRole("button", { name: "Lưu bản nháp", exact: true }).click();
    await expect(page.getByText("Giải thích được giáo viên sửa trong bản nháp.", { exact: false })).toBeVisible();
    await page.getByRole("button", { name: "Công bố", exact: true }).click();
    await page.getByRole("dialog").getByRole("button", { name: "Công bố", exact: true }).click();
    ...
    const assignmentUrl = contentUrl.replace(/\/details$/, "");
    await page.getByRole("button", { name: "Danh sách bài Quiz", exact: true }).click();
    await page.getByRole("row", { name: "Mở bài Quiz Kiểm tra chương 1", exact: true }).getByRole("button", { name: "Chi tiết", exact: true }).click();
    await expect(page).toHaveURL(assignmentUrl);
    await expect(page.getByText("15 phút", { exact: true })).toBeVisible();
    await expect(page.getByRole("region", { name: "Nội dung Quiz đã giao" })).toHaveCount(0);
    await page.goto(contentUrl);
    await expect(page.getByRole("button", { name: "Chi tiết bài giao", exact: true })).toHaveCount(0);
    const assignedContent = page.getByRole("region", { name: "Nội dung Quiz đã giao" });
    await expect(assignedContent).toContainText("Khóa chính có vai trò nào trong bảng dữ liệu?");
    await expect(assignedContent.locator(".ws-question-preview .correct")).toHaveCount(2);
    ...
    expect((await request.delete(`${base}/quizzes/${quiz.id}`, { headers: teacherHeaders, data: {} })).status()).toBe(200);
    await page.reload();
    await expect(assignedContent).toContainText("Khóa chính có vai trò nào trong bảng dữ liệu?");
    await expect(assignedContent).not.toContainText("Nội dung mới");
    await expect(page).toHaveURL(contentUrl);
    await student.goto(`${assignmentUrl}/details`);
    await student.reload();
    await expect(student.getByRole("region", { name: "Nội dung Quiz đã giao" })).toHaveCount(0);
    ...
    await page.getByRole("button", { name: "Xóa bài giao", exact: true }).click();
    await page.getByRole("dialog").getByRole("button", { name: "Xóa bài giao", exact: true }).click();
    await expect(page).toHaveURL(/#\/assignments$/);
    await expect(page.getByRole("row").filter({ hasText: "Kiểm tra chương 1" })).toHaveCount(0);
    await student.goto(assignmentUrl);
    await student.reload();
    await expect(student.getByRole("heading", { name: "Bài giao không khả dụng" })).toBeVisible();
    await student.goto("http://localhost:5175/#/results");
    await expect(student.getByRole("row").filter({ hasText: "Kiểm tra chương 1" })).toHaveCount(0);
  } finally {

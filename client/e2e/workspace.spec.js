...
test("student documents stay personal and class files remain read-only", async ({ page }) => {
  ...
  await page.getByRole("navigation", { name: "Điều hướng chính" }).getByRole("link", { name: "Học liệu của tôi", exact: true }).click();
  await expect(page.getByText("Cơ sở dữ liệu — Chương 2.pdf", { exact: true })).toHaveCount(0);
  await expect(page.getByRole("button", { name: "Tài liệu lớp học", exact: true })).toHaveCount(0);
  await page.getByRole("navigation", { name: "Điều hướng chính" }).getByRole("link", { name: /^Lớp học/ }).click();
  await page.locator(".ws-class-card").filter({ hasText: "Cơ sở dữ liệu" }).getByRole("button", { name: "Vào lớp", exact: true }).click();
  await page.getByRole("button", { name: "Tài liệu lớp", exact: true }).click();
  await page.getByText("Cơ sở dữ liệu — Chương 2.pdf", { exact: true }).click();
...
test("teacher can create a class, publish an assignment and review requests", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Quiz được giao", exact: true }).click();
  await page.getByRole("button", { name: "Giao Quiz", exact: true }).click();
  const defaultStartAge = await page.getByLabel("Thời gian mở bài").evaluate((input) => Date.now() - new Date(input.value).getTime());
  expect(defaultStartAge).toBeGreaterThanOrEqual(0);
  expect(defaultStartAge).toBeLessThan(60000);
  ...
  await page.locator(".ws-class-card").filter({ has: page.getByRole("heading", { name: "Cơ sở dữ liệu", exact: true }) }).locator(".ws-class-card-cover").click();

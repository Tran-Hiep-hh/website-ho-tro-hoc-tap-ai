...
before(async () => {
  ...
  const notificationMigration = await readFile(new URL("../../database/migrations/008_notifications.sql", import.meta.url), "utf8");
  await database.query(notificationMigration); await database.query(notificationMigration);
...
test("Notifications follow class events, isolate recipients and persist read state", async () => {
  const cls = await classroom();
  assert.equal((await fetch(`${base}/notifications`)).status, 401);
  const route = `classes/${cls.id}`;
  await call(`/classes/${cls.id}/join`, "POST", { code: cls.code }, 2);
  await call(`/classes/${cls.id}/join`, "POST", { code: cls.code }, 2);
  const received = (await call("/notifications")).notifications.filter((n) => n.route === `${route}/requests`);
  assert.equal(received.length, 1); assert.equal(received[0].read, false);
  assert.equal((await call(`/notifications/${received[0].id}/read`, "PUT", {}, 2)).status, 404);
  await call(`/notifications/${received[0].id}/read`, "PUT", {});
  assert.equal((await call("/notifications")).notifications.find((n) => n.id === received[0].id).read, true);
  let req = (await call("/classes")).requests.find((r) => r.classId === cls.id);
  await call(`/classes/${cls.id}/requests/${req.id}`, "POST", { approve: false });
  assert.ok((await call("/notifications", "GET", undefined, 2)).notifications.some((n) => n.title === "Yêu cầu tham gia bị từ chối" && n.text.includes(cls.name)));
  await call(`/classes/${cls.id}/join`, "POST", { code: cls.code }, 2);
  req = (await call("/classes")).requests.find((r) => r.classId === cls.id);
  await call(`/classes/${cls.id}/requests/${req.id}`, "POST", { approve: true });
  await call(`/classes/${cls.id}/requests/${req.id}`, "POST", { approve: true });
  const approved = (await call("/notifications", "GET", undefined, 2)).notifications.filter((n) => n.route === route);
  assert.equal(approved.length, 1);
  const doc = await document();
  await call(`/classes/${cls.id}/materials`, "POST", { documentIds: [doc.id] });
  await call(`/classes/${cls.id}/materials`, "POST", { documentIds: [doc.id] });
  const student = (await call("/notifications", "GET", undefined, 2)).notifications.filter((n) => n.route === route);
  assert.equal(student.length, 2);
  assert.ok(!(await call("/notifications", "GET", undefined, 3)).notifications.some((n) => n.route === route));
  await call("/notifications/read", "PUT", { throughId: approved[0].id }, 2);
  const after = (await call("/notifications", "GET", undefined, 2)).notifications;
  assert.equal(after.find((n) => n.id === approved[0].id).read, true);
  assert.equal(after.find((n) => n.id === student[0].id).read, false);
  assert.ok(!(await call("/notifications", "GET", undefined, 1)).notifications.some((n) => n.route.startsWith(route)));
});
